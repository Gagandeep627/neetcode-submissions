class Solution:
    def maxArea(self, heights: List[int]) -> int:
        ...
        # now lets check height (corrosponding start will be less than end)
        # now validate if height(start) is lesser than height(end)
        # it means :height[start +1] may be higher than height[start]
        # as the area will be dependable to mins(height[start], height[end])
        # so lets have a hypothetical supposition that
        # heights corrosponding (start + 1) may have the higher value
        # heights (start);
        ...
        # and in similar way suppose heights(end) is lower than height(start)
        # means : now lets suppose heights[end-1] will be higher than heights[end]
        #         end -= 1
        ...
        for i in range(len(heights)):
            ...
            for j in range(i + 1, len(heights)):
                ...
                #answer : res
                # take the mins(height[i], height[j]) #height of the container possible 
                # width of the container :(end- start) : (j-i)
                # multiplication of both above will lead to container with the most water
                # maintain the container  with the most of water
                # via max functions (res, calculated (3)) watre container for the bucket so far).
                # maintain the maximum container filled with water so far required..
                # stire above in res;
        ...
        # return answer ;set res : answer
        # time_complexity :O(n * n) : O(n ^ 2); : O(n * n) : O(n ^ 2);
        # return answer;
        return res
