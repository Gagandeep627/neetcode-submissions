class Solution:
    ...
    # for memoized solutions-->
#         Time Complexity	O(n)	Each index i is computed once and memoized.
# Space Complexity	O(n)	For recursion stack (O(n)) + dp dictionary (O(n)).
        res = robbing(nums, 0)
        return res
...
# Every house has 2 choices → rob or skip.

# For n houses → exponential recursion tree.
